...
function init_tabs() {
    ...
    /*	GROUPS {
            name : {
                tabsets: [setOfLis, setOfLis, setOf...]
                tabpanes: nodes, can be many .tab-pane from different panels
            }
        }*/
    ...
    /**
     *
     * @returns object { target : {sets: [], panes: targetPanes} }
     */
...
function _discover_tabsets() {
    ...
    tabSets.forEach( tabSet => {
        ...
        if (!targetSets[ttarget])
            targetSets[ttarget] = {tabsets: [], tabpanes: targetPanes}

        targetSets[ttarget].tabsets.push(actualTabs);
    });
